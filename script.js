...
const NODES = [
  { id:"reel",     label:"Showreel",  sub:"watch the reel →", x:130, y:70,  color:"var(--accent-green)",  target:"#reel", fixed:true, tooltip:"the Showreel" },
  { id:"booth",    label:"ishita_sehgal.mov", sub:"", x:130, y:220, w:120, h:120, color:"var(--accent-purple)", thumb:"greenscreen-placeholder.jpg", fixed:true, image:true, tooltip:"the Booth" },
  { id:"projects", label:"Projects",  sub:"see the work →",   x:130, y:440, color:"var(--accent-green)",  target:"#projects", fixed:true, tooltip:"Projects" },
  { id:"about",    label:"About",     sub:"who I am →",       x:490, y:270, color:"var(--accent-coral)", target:"#about", fixed:true, tooltip:"About" },
  { id:"contact",  label:"Contact",   sub:"say hello →",      x:850, y:270, color:"var(--accent-cyan)",  target:"#contact", fixed:true, tooltip:"Contact" },
];

const EDGES = [
  ["reel","about"], ["booth","about"], ["projects","about"], ["about","contact"],
];
...
function portOut(n){ const w=n.w||NODE_W, h=n.h||NODE_H; return { x: n.x + w, y: n.y + h/2 }; }
function portIn(n){ const h=n.h||NODE_H; return { x: n.x, y: n.y + h/2 }; }
...
function renderGraph(){
  ...
  allNodes().forEach(n => (n.image ? buildImageNode(n, nodesLayer) : buildNode(n, nodesLayer)));
}

// shared tooltip element used by every fixed node ("click to navigate to X")
const nodeTooltip = document.getElementById("nodeTooltip");
function showNodeTooltip(text, x, y){
  if (!text) return;
...
function hideNodeTooltip(){ nodeTooltip.classList.remove("is-visible"); }

function attachNodeHoverBehavior(g, n){
  const tipText = n.tooltip ? `click to navigate to ${n.tooltip}` : null;
  g.addEventListener("mouseenter", e => {
    highlightEdges(n.id, true);
    showNodeTooltip(tipText, e.clientX, e.clientY - 14);
  });
  ...
  g.addEventListener("focus", () => {
    ...
    showNodeTooltip(tipText, r.left + r.width/2, r.top - 8);
  });
...
// a "Read"-style node that shows an actual thumbnail (like footage in Nuke's
// node graph) instead of a plain label. Clicking it opens a floating panel
// rather than scrolling — it has no page section of its own.
function buildImageNode(n, nodesLayer){
  const w = n.w || NODE_W, h = n.h || NODE_H;
  const g = document.createElementNS(svgNS, "g");
  g.setAttribute("class", "node node--image");
  g.setAttribute("tabindex", "0");
  ...
  img.setAttributeNS("http://www.w3.org/1999/xlink", "href", n.thumb);
  img.setAttribute("href", n.thumb);
  img.setAttribute("x", n.x); img.setAttribute("y", n.y);
  img.setAttribute("width", w); img.setAttribute("height", h);
  img.setAttribute("preserveAspectRatio", "xMidYMid slice");
  img.setAttribute("clip-path", `url(#${clipId})`);
  img.setAttribute("class", "node__thumb-img");
  g.appendChild(img);

  const capH = 20;
  const cap = document.createElementNS(svgNS, "rect");
  cap.setAttribute("x", n.x); cap.setAttribute("y", n.y + h - capH);
  cap.setAttribute("width", w); cap.setAttribute("height", capH);
  cap.setAttribute("clip-path", `url(#${clipId})`);
  cap.setAttribute("class", "node__caption-bg");
  g.appendChild(cap);

  const bar = document.createElementNS(svgNS, "rect");
  bar.setAttribute("x", n.x); bar.setAttribute("y", n.y);
  bar.setAttribute("width", w); bar.setAttribute("height", BAR_H);
  ...
  const label = document.createElementNS(svgNS, "text");
  label.setAttribute("x", n.x + 8); label.setAttribute("y", n.y + h - 6);
  label.setAttribute("class", "node__thumb-label");
  label.textContent = n.label;
  g.appendChild(label);

  // stroke-only border on top, reuses the same hover/focus styling as other nodes
  const border = document.createElementNS(svgNS, "rect");
  border.setAttribute("x", n.x); border.setAttribute("y", n.y);
  border.setAttribute("width", w); border.setAttribute("height", h);
  border.setAttribute("rx", 3);
  border.setAttribute("class", "node__body node__body--image");
  g.appendChild(border);

  const badge = document.createElementNS(svgNS, "circle");
  badge.setAttribute("cx", n.x + w - 15);
  badge.setAttribute("cy", n.y + 15);
  badge.setAttribute("r", 10);
  badge.setAttribute("class", "node__badge");
  g.appendChild(badge);

  const chevron = document.createElementNS(svgNS, "path");
  const cx = n.x + w - 15, cy = n.y + 15;
  chevron.setAttribute("d", `M ${cx-3} ${cy-5} L ${cx+4} ${cy} L ${cx-3} ${cy+5}`);
  chevron.setAttribute("class", "node__chevron");
  g.appendChild(chevron);

  function activate(){ openBoothPanel(); }
  border.addEventListener("pointerdown", e => startNodeMove(e, n, activate));
  g.addEventListener("keydown", e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); activate(); } });
...
(function buildProjects(){
  ...
  PROJECTS.forEach((p, i) => {
    ...
    card.setAttribute("aria-label", `${p.name} — flip or click for details`);
    card.innerHTML = `
      <div class="project-card__inner">
        ...
        <div class="project-card__back">
          <h3 class="project-card__back-name">${p.name}</h3>
          <p class="project-card__back-desc">${p.desc}</p>
          <div class="project-card__back-tags">${p.tags.map(t => `<span>${t}</span>`).join("")}</div>
          <span class="project-card__back-hint">click for more →</span>
        </div>
      </div>`;
...
// Uses an actual photo (greenscreen-placeholder.jpg) — replace
...
boothPhoto.onerror = () => {
  console.warn("Keyer Booth: couldn't load greenscreen-placeholder.jpg");
};
boothPhoto.src = "greenscreen-placeholder.jpg";
...
// ---- background swatches + custom color picker + upload ----
...
BOOTH_BACKGROUNDS.forEach((bgOpt, i) => {
  ...
  btn.addEventListener("click", () => {
    ...
    btn.classList.add("is-active");
    boothCustomSwatch.classList.remove("is-active");
    redrawBooth();
  });
  boothSwatchesEl.appendChild(btn);
});

// native color picker (wheel + hex input, browser-provided) for any exact
// constant color, not just the presets above
const boothCustomSwatch = document.createElement("label");
boothCustomSwatch.className = "booth__custom-swatch";
boothCustomSwatch.title = "custom color";
boothCustomSwatch.innerHTML = `<input type="color" id="boothCustomColor" value="#7f8fa6">`;
boothSwatchesEl.appendChild(boothCustomSwatch);

const boothCustomColorInput = document.getElementById("boothCustomColor");
boothCustomColorInput.addEventListener("input", e => {
  boothState.bg = { type:"color", value:e.target.value, img:null };
  boothSwatchesEl.querySelectorAll("button").forEach(b => b.classList.remove("is-active"));
  boothCustomSwatch.classList.add("is-active");
  redrawBooth();
});
...
// ---- floating panel open/close (triggered only by the Booth node) ----
const boothOverlay = document.getElementById("boothOverlay");

function openBoothPanel(){
  boothOverlay.classList.add("is-open");
  boothOverlay.setAttribute("aria-hidden", "false");
  document.body.style.overflow = "hidden";
}
function closeBoothPanel(){
  boothOverlay.classList.remove("is-open");
  boothOverlay.setAttribute("aria-hidden", "true");
  document.body.style.overflow = "";
}
boothOverlay.querySelectorAll("[data-close-boothpanel]").forEach(el =>
  el.addEventListener("click", closeBoothPanel)
);
document.addEventListener("keydown", e => {
  if (e.key !== "Escape") return;
  // close whichever is on top first
  if (boothModal.classList.contains("is-open")) { closeBoothModal(); return; }
  if (boothOverlay.classList.contains("is-open")) { closeBoothPanel(); }
});
